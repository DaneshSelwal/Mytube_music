...
## 2024-05-24 - LrcParser String Allocation Optimization
**Learning:** Frequent case-insensitive string parsing on large formats (like LRC files) using Regex and multiple string allocations (like `substring()` or `trim()`) can create substantial performance bottlenecks due to string allocation overhead.
**Action:** When performing string parsing with highly predictable formats, prefer manual character index checks, manual whitespace iteration, and primitive mathematical conversion over `Regex` usage and `substring()` allocations to significantly reduce processing time and GC overhead.
